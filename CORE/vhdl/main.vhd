...
begin
  ...
  -- $DExx / $DFxx writes only land in the cart's IO scratch RAM when the
  -- cartridge says so via crt_iox_wr_ena. Carts whose IO area is just a
  -- control register (e.g. Action Replay $DE00) leave the ena low so the
  -- write is consumed only by their case-statement side effects.
  crt_addr_9exx   <= '1' when c64_ram_addr_o(12 downto 8) = "11110" else '0';
  crt_addr_9fxx   <= '1' when c64_ram_addr_o(12 downto 8) = "11111" else '0';

  crt_ioe_we_o    <= crt_ioe_wr_ena and (core_ioe or (core_roml and crt_addr_9exx)) and c64_ram_we;
