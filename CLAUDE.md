# C64MEGA65 — Project Guide for Claude

This file is the cold-start brief. Read it first; it covers what this project
*is*, what its conventions look like, and where to find things — so you can
spend context on the task instead of rediscovering the layout.
...
### MiSTer2MEGA65 web research confirmation

I verified my tools could fetch the M2M repo by hitting the README and Wiki
landing/index pages. Several wiki sub-pages (QNICE, Virtual-Drives,
On-Screen-Menu, Understand-the-MiSTer-core) are explicitly marked **WIP/@TODO**
on GitHub — for those topics, this CLAUDE.md is more authoritative than the
upstream wiki, because the answers had to come from reading the actual code.
...
`/c64/c64mega65` file with `M2M/tools/make_config.sh c64mega65 auto` (size
must match `OPTM_SIZE` from `config.vhd`).
